...
void ClockWindow::Draw(const std::string& windowName, bool* open)
{
    ImGui::SetNextWindowPos(this->GetNextPos());
    ImGui::SetNextWindowSize(this->GetNextSize());

    ImGui::Begin(windowName.c_str(), nullptr, kMainWindowFlags);
...
void ClockWindow::drawCircle(const float kRadius)
{
    ImGui::GetWindowDrawList()->AddCircle(_center, kRadius, ImGui::GetColorU32(ImGuiCol_Text), 100, 2.0f);
}

void ClockWindow::drawClockHand(const float kRadius, const float kTheta, const ImColor kColor)
{
...
void ClockWindow::drawAllHourStrokes()
{
...
void ClockWindow::drawAllMinuteStrokes()
{
...
void ClockWindow::drawDigitalClock()
{
    ImGui::Text("%02d:%02d:%02d", _hrs, _mins, _secs);
}

void ClockWindow::GetTime()
{
...
std::tuple<float, float, float> ClockWindow::GetTheta()
{
